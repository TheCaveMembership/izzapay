...
# Optional: use an already funded test wallet to create/fund new accounts
...
def horizon_account_exists(pubkey: str) -> bool:
    try:
        _ = server.accounts().account_id(pubkey).call()
        return True
...
def maybe_create_account(target_pub: str):
    ...
    if not FUNDING_SECRET:
        raise RuntimeError(
            ...
            f"→ Fund/activate it first (Pi Testnet) then re-run.\n"
            f"  Faucet (if enabled): {HORIZON_URL.replace('/horizon','')}/friendbot?addr={target_pub}\n"
            f"  Or send a CreateAccount from your Pi Test Wallet."
        )
    # Build create_account from the funding key
