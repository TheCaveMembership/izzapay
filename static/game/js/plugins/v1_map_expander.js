...
(function () {
  const BUILD = 'v2.9-map-expander+downtown-stitch+behind-core+no-overwrite';
  console.log('[IZZA PLAY]', BUILD);

  const MAP_TIER_KEY = 'izzaMapTier';             // '1' | '2'
  const TIER2 = { x0: 10, y0: 12, x1: 80, y1: 50 }; // expanded play box (core)
  const TIER1 = { x0: 18, y0: 18, x1: 72, y1: 42 }; // original area (do NOT paint over)
  ...
  function coreHRoadY() {
    ...
    return doorGY + 1; // matches core: sidewalkTopY = hRoadY - 1
  }
  function coreVRoadX() {
    const doorGX = Math.floor((api.doorSpawn.x + 8) / api.TILE);
    return doorGX + 11; // matches core derivation
  }

  // clamp helpers
  const clampSegOutside = {
    // returns up to two [a,b] ranges from x0..x1 that are OUTSIDE tier1.x0..tier1.x1
    x(x0, x1) {
      const out = [];
    ...
    // returns up to two [a,b] ranges from y0..y1 that are OUTSIDE tier1.y0..tier1.y1
    y(y0, y1) {
      const out = [];
  ...
  // ---- Downtown layout derived from core spines ----
  function buildLayout() {
    const HR = coreHRoadY();
    const VX = coreVRoadX();

    // rows below the main road
    const rows = [HR + 6, HR + 12, HR + 18, HR + 24];
    // columns around the avenue
    const cols = [VX - 16, VX - 8, VX, VX + 10, VX + 22];

    // Horizontal roads: draw from outside Tier1 to edges, so we stitch cleanly
    ...
    // Vertical roads: same idea, but clip vertically outside Tier1
    const V_ROADS = cols.map((x) => ({
      x,
      y0: rows[0] - 1,                  // abuts the main road band
      y1: rows[rows.length - 1] + 1
    }));

    // Buildings (all outside Tier1)
    const BUILDINGS = [
      // north band
  ...
  function notIntersectTier1(b) {
    ...
    // also keep everything inside TIER2
  ...
  function drawMainOverlay(layout) {
    ...
    ctx.save();
    // >>> paint BEHIND the core & sprites
    ctx.globalCompositeOperation = 'destination-over';

    // 1) base grass ONLY outside Tier1
    for (let gy = TIER2.y0; gy <= TIER2.y1; gy++) {
      for (let gx = TIER2.x0; gx <= TIER2.x1; gx++) {
        const inTier1 = gx >= TIER1.x0 && gx <= TIER1.x1 && gy >= TIER1.y0 && gy <= TIER1.y1;
        if (!inTier1) fillTile(ctx, gx, gy, COL.grass);
      }
    }

    // 2) sidewalks (clip around Tier1), then roads (clip)
    layout.H_ROADS.forEach((r) => {
      // sidewalk bands
      clampSegOutside.x(r.x0, r.x1).forEach(([a, b]) => {
        drawSidewalkRow(ctx, r.y - 1, a, b);
        drawSidewalkRow(ctx, r.y + 1, a, b);
      });
      // road
      clampSegOutside.x(r.x0, r.x1).forEach(([a, b]) => drawHRoadSegment(ctx, r.y, a, b));
    });

    layout.V_ROADS.forEach((r) => {
      clampSegOutside.y(r.y0, r.y1).forEach(([a, b]) => {
        drawSidewalkCol(ctx, r.x - 1, a, b);
        drawSidewalkCol(ctx, r.x + 1, a, b);
      });
      clampSegOutside.y(r.y0, r.y1).forEach(([a, b]) => drawVRoadSegment(ctx, r.x, a, b));
    });

    // 3) solids + lakes (only those we kept outside Tier1)
  ...
  // ---- minimap / bigmap (unchanged logic) ----
  ...
  IZZA.on('render-post', () => {
    ...
    drawMainOverlay(layout); // behind core
    drawMini(layout);        // minimap/bigmap keep matching
  });
})();
