...
(function () {
  ...
  const LAKE   = { x0:67, y0:35, x1:81, y1:49 }; // within Tier-2 bounds
  ...
  function proposeDowntown(a){
    ...
    const Hcand = [ hRoadY-6, hRoadY, hRoadY+6 ].map(y=>({y, x0:L, x1:R}));
    const Vcand = [ vRoadX-9, vRoadX, vRoadX+9 ].map(x=>({x, y0:T, y1:B}));

    // Buildings inside blocks (sidewalks added later)
    const BLD = [
  ...
  function makeSafeLayout(a){
    ...
    // Keep roads out of HQ/Shop/doors and WATER
    const NO_ROAD = [
    ...
    // Buildings: not near HQ/Shop and never in water
    ...
    // Roads clipped against buildings & no-road regions (so they never plow into walls/water)
  ...
  IZZA.on('render-under', ()=>{
    ...
    const mark = (gx,gy)=>{ const k=gx+'|'+gy; if(seenSW.has(k)) return false; seenSW.add(k); return true; };
    ...
    // 4) Buildings (never on roads/sidewalks/water)
    ...
    // 5) Downtown park — CLIPPED so it never paints over roads/sidewalks/water
    if(_layout.PARK){
    ...
    // Boats (visuals) — draw after water
    ...
    if(_towBoat){
      ...
      // wakeboarder (stays on the lake; not drawn on minimap)
  ...
  // Water guard + car ↔ building avoidance + boat follow
  IZZA.on('update-pre', ({dtSec})=>{
    if(!IZZA.api?.ready || !isTier2()) return;
  ...
  function paintOverlay(id){
    ...
    ctx.fillRect(LAKE.x0*sx,LAKE.y0*sy,(LAKE.y1-LAKE.y0+1)*sy,(LAKE.x1-LAKE.x0+1)*sx); // swapped to width/height below
    ctx.fillRect(LAKE.x0*sx,LAKE.y0*sy,(LAKE.x1-LAKE.x0+1)*sx,(LAKE.y1-LAKE.y0+1)*sy);   // actual lake rect
