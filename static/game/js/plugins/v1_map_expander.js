...
(function () {
  ...
  function anchors(api){
    ...
    const hRoadY       = bY + bH + 1;         // main horizontal
    const sidewalkTopY = hRoadY - 1;

    const vRoadX         = Math.min(un.x1-3, bX + bW + 6); // main vertical
  ...
  function proposeDowntown(a){
    ...
    // arterials + cross streets (simple, cleaner)
    H.push({y:hRoadY, x0:L, x1:R});
    H.push({y:hRoadY-8, x0:L, x1:R});
    H.push({y:hRoadY+8, x0:L, x1:R});

    V.push({x:vRoadX, y0:T, y1:B});
    V.push({x:vRoadX-10, y0:T, y1:B});
    V.push({x:vRoadX+10, y0:T, y1:B});

    // a few landmark buildings far from HQ/Shop and sidewalks
    addBox(vRoadX+8,  hRoadY+10, 8,5, COL.shop);
    addBox(vRoadX-14, hRoadY+2,  5,3, COL.civic);
    addBox(vRoadX+14, hRoadY-6,  4,3, COL.police);
  ...
  function makeSafeLayout(a){
    ...
    // base clip
    let H = P.H.flatMap(seg => clipHSegment(seg, NO_ROAD));
    let V = P.V.flatMap(seg => clipVSegment(seg, NO_ROAD));

    // additional de-clutter per your notes:
    // - remove vertical above HQ
    V = V.filter(seg => !(seg.x===a.vRoadX && seg.y0 < a.hRoadY));
    // - remove vertical right behind the shop doorway area
    V = V.filter(seg => !(seg.x===a.vRoadX && seg.y0>=a.hRoadY && seg.y0<=a.hRoadY+6));
  ...
  IZZA.on('render-under', ()=>{
    ...
    // sidewalks first (only where there are actual roads)
    L.H_ROADS.forEach(r=>{
  ...
  // ---------- light collision against NEW buildings (+hotel + houses later) ----------
  function softCollide(extraRects=[]){
    ...
    const rects = [...L.BUILDINGS, ...extraRects].map(b=>({x:b.x,y:b.y,w:b.w,h:b.h}));
  ...
  // expose for the add-on section below
  window.__IZZA_SOFT_COLLIDE__ = softCollide;
})();
...
// Bottom-left neighborhood (little suburb)
...
const _isWater=(gx,gy)=> _inRect(gx,gy,LAKE) && !_isDock(gx,gy); // docks are walkable
...
IZZA.on('render-under', ()=>{
  ...
  // Lake (only big lake; no tiny roadside puddles)
  for(let gy=LAKE.y0; gy<=LAKE.y1; gy++)
    for(let gx=LAKE.x0; gx<=LAKE.x1; gx++)
      ctx && _isWater(gx,gy) && fillTile(api,ctx,gx,gy,'#1a4668');
  ...
  DOCKS.forEach(d=>{
    const S=api.DRAW, sx=(d.x0*api.TILE - api.camera.x)*(S/api.TILE), sy=(d.y*api.TILE - api.camera.y)*(S/api.TILE);
...
let _towBoat=null; let _inBoat=false, _ride=null, _lastLand=null;

function _spawnBoats(){
  if(!isTier2() || _boats.length) return;
  // rectangular loop inset so boats avoid docks
  const L={x0:LAKE.x0+5,y0:LAKE.y0+2,x1:LAKE.x1-2,y1:LAKE.y1-2};
...
IZZA.on('update-pre', ({dtSec})=>{
  ...
  _boats.forEach(b=>{
    ...
    // simple avoid: if next point would cross a dock row, skip to next corner
    if(DOCKS.some(d=> d.y===Math.round(b.y) && Math.sign(tgt.x-b.x)<0 ? (b.x>=d.x0 && tgt.x<=d.x0+d.len) : (b.x<=d.x0+d.len && tgt.x>=d.x0))){
      b.i=(b.i+1)%b.path.length;
    }
    const t2=b.path[b.i], DX=t2.x-b.x, DY=t2.y-b.y, M=Math.hypot(DX,DY)||1;
    if(M<=step){ b.x=t2.x; b.y=t2.y; b.i=(b.i+1)%b.path.length; }
...
// enter/exit boat on B while on a dock (walkable) or on beach to exit
function _enterBoat(){
...
// draw boats + wakeboarder + rope under sprites
IZZA.on('render-under', ()=>{
  if(!isTier2() || !IZZA.api?.ready) return;
  ...
  // wakeboarder with rope to towing boat
  if(_towBoat){
    const tgt=_towBoat.path[_towBoat.i], vx=tgt.x-_towBoat.x, vy=tgt.y-_towBoat.y, m=Math.hypot(vx,vy)||1;
    const wx=_towBoat.x - (vx/m)*2.4, wy=_towBoat.y - (vy/m)*2.4;
    // rope
    ctx.strokeStyle='rgba(255,255,255,0.8)';
    ctx.lineWidth=2;
    ctx.beginPath();
    ctx.moveTo(sx(_towBoat.x)+S*0.5, sy(_towBoat.y)+S*0.5);
    ctx.lineTo(sx(wx)+S*0.5,       sy(wy)+S*0.5);
    ctx.stroke();
    // rider
...
// ===== cars vs buildings: keep cars out of buildings =====
IZZA.on('update-post', ()=>{
  if(!isTier2() || !IZZA.api?.ready) return;
...
// ===== include hotel + houses in collision to prevent “walk under building” =====
IZZA.on('update-post', ()=>{
  if(!isTier2() || !IZZA.api?.ready) return;
  const extra = [
    {x:HOTEL.x0,y:HOTEL.y0,w:HOTEL.x1-HOTEL.x0+1,h:HOTEL.y1-HOTEL.y0+1},
    ...HOUSES.map(h=>({x:h.x0,y:h.y0,w:h.x1-h.x0+1,h:h.y1-h.y0+1}))
  ];
  window.__IZZA_SOFT_COLLIDE__?.(extra);
});
