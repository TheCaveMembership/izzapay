...
(function(){
  ...
  // ---------- tiny, safe confirm (custom popup with wood text + stronger skull bg) ----------
  function confirmPickup(cb){
    try{
      // wood grain SVG data-uri for text fill
      const woodTex = encodeURIComponent(`
        <svg xmlns="http://www.w3.org/2000/svg" width="120" height="60">
          <defs>
            <linearGradient id="g" x1="0" x2="1">
              <stop offset="0" stop-color="#7b5a2b"/>
              <stop offset="0.5" stop-color="#a0723c"/>
              <stop offset="1" stop-color="#6b4a2f"/>
            </linearGradient>
          </defs>
          <rect width="100%" height="100%" fill="url(#g)"/>
          <g opacity="0.35" stroke="#442f17" stroke-width="2" fill="none">
            <path d="M0,20 C30,10 60,30 120,18"/>
            <path d="M0,40 C25,32 70,46 120,36"/>
          </g>
        </svg>
      `);
      ...
      wrap.style.cssText =
        ...
        'background:rgba(0,0,0,.55);z-index:10000;'; // ↑ ensure on top of chips
      ...
      title.style.cssText =
        ...
        'text-shadow:0 1px 0 rgba(255,255,255,0.35), 0 2px 0 rgba(0,0,0,0.15);';
      title.textContent = 'THE BOX CALLS.';

      const rhyme = document.createElement('div');
      rhyme.style.cssText = 'margin:10px 0 14px;font-weight:900;line-height:1.35;' +
        `background-image:url("data:image/svg+xml,${woodTex}"); -webkit-background-clip:text; background-clip:text; color:transparent;` +
        'text-shadow:0 1px 0 rgba(255,255,255,0.25), 0 2px 0 rgba(0,0,0,0.12);';
      rhyme.innerHTML =
        `<div style="transform:rotate(-0.4deg)">` +
        `Cardboard crown, street-born throne — steal the hush, claim your zone.<br>` +
        `Tap in, fade loud… or leave it lone.<br>` +
        `Ever slide the docks, drove a boat alone?` +
        `</div>`;
