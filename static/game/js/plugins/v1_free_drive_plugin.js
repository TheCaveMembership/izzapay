...
(function(){
  ...
  function ensureCopsMatchWanted(){
  try{
    const need = (api.player && (api.player.wanted|0)) || 0;
    const have = (api.cops && api.cops.length) || 0;
    if (need <= 0){
      if (api.cops && api.cops.length) api.cops.length = 0;
      return;
    }
    if (have < need){
      for(let i=have; i<need; i++){ spawnCopNearPlayer('police'); }
    } else if (have > need){
      api.cops.length = need;
    }
  }catch(e){ console.warn('[free_drive] ensureCopsMatchWanted failed', e); }
}
