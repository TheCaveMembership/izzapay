...
(function(){
  ...
  const TRACER = {
    fire: {
      ...
      colors: {
        glow0: 'rgba(255,180,60,',
        glow1: 'rgba(255,120,40,',
        core:  'rgba(255,210,120,',
        streak:'rgba(255,140,60,'
      }
    },
    neon: {
  ...
  function craftedGunTuning(){
    ...
    return {
      ...
      critChance: Math.max(0, Math.min(0.5, Number(it.critChance||0))),
  ...
  /* Auto loop for creator guns:
     - If the crafted gun is flagged auto, default interval = Uzi cadence.
     - Otherwise use the item's own fireIntervalMs (semi-auto behavior elsewhere). */
  ...
  function selectedCreatorFX(){
    ...
    if (/comet/.test(raw))                         return 'spark';
  ...
  /* Wherever you register a hit on NPC/PvP, use computeBulletDamageBase():
  ...
  */
  ...
  function uziStart(){
    ...
    uziTimer = setInterval(()=>{
      ...
      const spawned2 = spawnBulletOrPointBlank();
      if (!spawned2){
        const inv = readInv();
  ...
  function ensureFireBtn(){
    ...
    const up  =(ev)=>{ ev.preventDefault(); ev.stopPropagation(); uziStop(); creatorAutoStop(); /* melee one-shot */ };
  ...
  function attachHooks(){
    ...
    function drawMeleeWeaponSwing(ctx){
      ...
      if (img && img.complete){
        try{ ctx2.drawImage(img, -w/2, -h/2, w, h); }catch{}
      } else {
    ...
    IZZA.on('render-post', ()=>{
      ...
      // --- FX Events (only for creator-gun bullets with a selected tracer) ---
      for(let i=fxEvents.length-1;i>=0;i--){
        const fx = fxEvents[i];
        const age = now()-fx.born;
        if(age > 550){ fxEvents.splice(i,1); continue; }
