// PvP Duel Client — v7.9 stable
// - Keeps MP client event flow: duel-config, duel-round-start, duel-round-end, duel-match-finish
// - Safer best-of-3 round fencing
// - Smooth opponent interpolation
// - Cleaner duel cleanup
// - Does not interfere with Worlds remote-player layer
(function(){
  if(window.__IZZA_PVP_DUEL_V79__) return;
  window.__IZZA_PVP_DUEL_V79__ = true;

  const BUILD='v7.9-duel-client-stable';
  ...
  async function $post(p,b){
    try{
      const r = await fetch(withTok(BASE+p), {
        method:'POST',
        credentials:'include',
  ...
  async function $get(p){
    try{
      const r = await fetch(withTok(BASE+p), {credentials:'include'});
      return r.ok ? r.json() : null;
    }catch{ return null; }
  }
  ...
  function hash01str(str){
    return (hash32(String(str)) % 100000) / 100000;
  }
  ...
  function buildSpawnCandidates(api){
    ...
    const hRoadY = api.hRoadY || ((un.y0 + un.y1) / 2 | 0);
    const vRoadX = api.vRoadX || ((un.x0 + un.x1) / 2 | 0);
    const sidewalkTopY = hRoadY - 1;
    const sidewalkBotY = hRoadY + 1;
    const leftX = vRoadX - 1;
    const rightX = vRoadX + 1;
    ...
    const cand=[];
    linspace(un.x0+3, un.x1-3, 10).forEach(gx=>cand.push({x:gx*t,y:sidewalkTopY*t,facing:'down',band:'top'}));
    linspace(un.x0+3, un.x1-3, 10).forEach(gx=>cand.push({x:gx*t,y:sidewalkBotY*t,facing:'up',band:'bot'}));
    linspace(un.y0+3, un.y1-3, 10).forEach(gy=>cand.push({x:leftX*t,y:gy*t,facing:'right',band:'left'}));
    linspace(un.y0+3, un.y1-3, 10).forEach(gy=>cand.push({x:rightX*t,y:gy*t,facing:'left',band:'right'}));
  ...
  function computeRoundSpawns(api, matchId, roundNum, sortedUsernames){
    ...
    const s1 = cand[Math.floor(hash01str(seed+'|p1') * cand.length)] || cand[0];
  ...
  function maxHearts(){
    return Math.max(1, parseInt(localStorage.getItem('izzaMaxHearts') || '3', 10));
  }
  ...
  function drawHeartsDOM(heartsFloat){
    const hud=document.getElementById('heartsHud');
    if(!hud) return;
    ...
    for(let i=0;i<mh;i++){
      ...
      rect.setAttribute('width', String(24*Math.max(0,Math.min(1,ratio))));
    ...
    if(!document.getElementById('heartBlinkCSS')){
      ...
      st.id='heartBlinkCSS';
      st.textContent=`@keyframes heartBlink{0%,100%{filter:drop-shadow(0 0 0 rgba(255,90,90,.0))}50%{filter:drop-shadow(0 0 10px rgba(255,90,90,.9))}} #heartsHud .heart-blink{animation:heartBlink .6s infinite}`;
      document.head.appendChild(st);
    }
  }

  function showBanner(text, kind='round'){
  ...
  function drawSprite(ctx, pack, facing, dx, dy, S){
    ...
    ctx.drawImage(pack.img,0,(DIR_INDEX[facing]||0)*32,32,32,dx,dy,S,S);
  }
  ...
  let DUEL = {
    ...
    oppName:'',
    oppSprite:null,
    opp:{x:0,y:0,facing:'down'},
    oppRender:{x:0,y:0,facing:'down'},
    oppHP:null,
    pollMs:120,
    timer:null,
    flip:false,
    ...
    myWins:0,
    oppWins:0,
    handledRounds:Object.create(null),
    matchFinishSent:false,
    roundStartSent:Object.create(null),
    pvpDamageHooked:false
  };
  ...
  async function poke(){
    const api=IZZA.api;
    if(!api?.ready || !DUEL.mid) return;

    await $post('/duel/poke',{
      matchId:DUEL.mid,
      x:api.player.x,
      y:api.player.y,
  ...
  function resolveWinnerForMe(round){
    ...
    if(round.youWon === true) return true;
    if(round.youLost === true) return false;
    if(wUser) return isMe(wUser);
  ...
  function emitRoundStart(roundNumber){
    const key=`${DUEL.mid}:start:${roundNumber}`;
    if(DUEL.roundStartSent[key]) return;
    DUEL.roundStartSent[key]=1;
  ...
  function emitMatchFinish(winnerName){
    if(DUEL.matchFinishSent) return;
    DUEL.matchFinishSent=true;
  ...
  async function pull(){
    ...
    if(j.opponent){
      ...
      DUEL.opp.x = j.opponent.x|0;
      DUEL.opp.y = j.opponent.y|0;
      DUEL.opp.facing = j.opponent.facing || 'down';
      ...
      if(!DUEL.oppSprite && IZZA.api.addRemotePlayer){
        DUEL.oppSprite = IZZA.api.addRemotePlayer({
          username:DUEL.oppName,
          appearance:j.opponent.appearance || {}
        });
      }
    }
    ...
    if(j.round) handleRoundState(j.round);
  }

  function sendSelfDownOnce(){
    if(DUEL.selfdownSent || !DUEL.mid) return;
    DUEL.selfdownSent=true;
    $post('/duel/selfdown',{matchId:DUEL.mid});
  }

  function handleRoundState(round){
    const rn = round.number|0;

    if(rn && rn !== (DUEL.lastRoundNum|0)){
      startRound(rn);
    }

    if(round.ended || round.matchOver){
      const roundId = round.roundId || `r${rn}`;
      const fenceKey = `${DUEL.mid}:${roundId}`;

      if(DUEL.handledRounds[fenceKey]) return;

      const iWon = resolveWinnerForMe(round);
      if(iWon === null) return;

      DUEL.handledRounds[fenceKey]=1;
      ...
      showBanner(
        round.matchOver ? (iWon ? 'MATCH WIN!' : 'MATCH LOSS') : (iWon ? 'ROUND WIN' : 'ROUND LOSS'),
        iWon ? 'win' : 'lose'
      );

      emitRoundEnd({roundId,winnerIsMe:!!iWon,winner:winnerName});

      if(round.matchOver || DUEL.myWins >= DUEL.roundsToWin || DUEL.oppWins >= DUEL.roundsToWin){
  ...
  function startRound(rn){
    ...
    DUEL.myHP = 4;
    DUEL.oppHP = 4;
    DUEL.downReported=false;
    DUEL.selfdownSent=false;
    DUEL.canFire=false;
    ...
    IZZA.api.player.x = mySpot.x;
    IZZA.api.player.y = mySpot.y;
    IZZA.api.player.facing = mySpot.facing || 'down';

    DUEL.opp.x = oppSpot.x;
    DUEL.opp.y = oppSpot.y;
    DUEL.opp.facing = oppSpot.facing || 'up';
    DUEL.oppRender.x = DUEL.opp.x;
    DUEL.oppRender.y = DUEL.opp.y;
    DUEL.oppRender.facing = DUEL.opp.facing;
  ...
  function bounceToLobby(){
    try{
      ...
      if(ds){
        IZZA.api.player.x = ds.x;
        IZZA.api.player.y = ds.y;
        IZZA.api.player.facing='down';
      }
    }catch{}
  ...
  function cleanupDuel(){
    ...
    try{
      if(DUEL.oppSprite && IZZA.api.removeRemotePlayer){
        IZZA.api.removeRemotePlayer(DUEL.oppSprite);
      }
    }catch{}

    document.getElementById('duelHold')?.remove();
    document.getElementById('pvpCountdown')?.remove();
    ...
    DUEL.oppName='';
    DUEL.oppSprite=null;
    DUEL.opp={x:0,y:0,facing:'down'};
    DUEL.oppRender={x:0,y:0,facing:'down'};
    DUEL.oppHP=null;
    DUEL.myHP=null;
    DUEL.oppCops=[];
    DUEL.oppEquipped='hand';
    DUEL.lastRoundNum=0;
    DUEL.usernames=['',''];
    DUEL.meIsA=false;
    DUEL.effects=[];
    ...
    DUEL.myWins=0;
    DUEL.oppWins=0;
    DUEL.handledRounds=Object.create(null);
    DUEL.matchFinishSent=false;
    DUEL.roundStartSent=Object.create(null);
    DUEL.flip=false;
  ...
  function oppCenter(){
    return {x:(DUEL.oppRender.x|0)+16,y:(DUEL.oppRender.y|0)+16};
  }

  function meCenter(){
    const p=IZZA.api.player;
    return {x:(p.x|0)+16,y:(p.y|0)+16};
  }
  ...
  function hitscan(kind){
    ...
    return {
      hit,
      hitPoint: hit ? {x:opp.x-dir.x*2,y:opp.y-dir.y*2} : {x:me.x+dir.x*Math.min(dist,maxDist),y:me.y+dir.y*Math.min(dist,maxDist)}
    };
  }

  function addTracer(from,to,life=110){
    DUEL.effects.push({kind:'tracer',x1:from.x,y1:from.y,x2:to.x,y2:to.y,t:0,life});
    if(DUEL.mid) $post('/duel/trace',{matchId:DUEL.mid,kind:'tracer',x1:from.x,y1:from.y,x2:to.x,y2:to.y});
  }
  ...
  function installHitHooks(){
    ...
    if(fire && !fire.__duelHooked){
      ...
      const down=()=>{
        ...
        if(k==='uzi'){
          const r=hitscan('uzi');
          addTracer(me,r.hitPoint);
          if(r.hit){ addSpark(r.hitPoint); sendHit('uzi'); }

          if(!uziTimer){
            uziTimer=setInterval(()=>{
              if(!DUEL.canFire) return;
              const r2=hitscan('uzi');
              const m2=meCenter();
              addTracer(m2,r2.hitPoint);
              if(r2.hit){ addSpark(r2.hitPoint); sendHit('uzi'); }
            },105);
          }
        }else if(k==='pistol'){
          const r=hitscan('pistol');
          addTracer(me,r.hitPoint);
          if(r.hit){ addSpark(r.hitPoint); sendHit('pistol'); }
        }else if(k==='grenade'){
          setTimeout(()=>{
            if(!DUEL.canFire) return;
            const r=hitscan('grenade');
            if(r.hit){ addSpark(r.hitPoint,220); sendHit('grenade'); }
          },900);
        }else{
          if(meleeInRange()){
            addSpark(oppCenter());
            sendHit(k==='bat' ? 'bat' : k==='knuckles' ? 'knuckles' : 'hand');
          }
        }
      };
    ...
    if(!window.__IZZA_DUEL_KEY_HOOKED__){
      window.__IZZA_DUEL_KEY_HOOKED__=true;
      window.addEventListener('keydown',e=>{
        if((e.key||'').toLowerCase()!=='a') return;
        if(!DUEL.mid || !DUEL.canFire) return;

        const k=equippedKind();
        if(k!=='pistol' && k!=='uzi' && k!=='grenade' && meleeInRange()){
          addSpark(oppCenter());
          sendHit(k==='bat' ? 'bat' : k==='knuckles' ? 'knuckles' : 'hand');
        }
  ...
  function applyPvpDamageSegments(nSegs){
    ...
    DUEL.myHP=Math.max(0,(hpBefore*3 - segs)/3);
    saveSegmentsFromHP(DUEL.myHP);
    drawHeartsDOM(DUEL.myHP);
  ...
  async function resolveMeUsername(){
    ...
    return IZZA?.api?.user?.username || window.__MP_LAST_ME?.username || '';
  }
  ...
  async function begin(payload){
    ...
    DUEL.usernames=[pA,pB].map(norm).sort();
    DUEL.oppName = meL===pAL ? (pB || 'Opponent') : (pA || 'Opponent');
    ...
    onceSceneReady(()=>{
      ...
      try{ IZZA.api.setWanted?.(0); }catch{}

      startRound(1);
      startPolling();
      setTimeout(installHitHooks,60);

      if(!DUEL.pvpDamageHooked && window.IZZA && IZZA.on){
        DUEL.pvpDamageHooked=true;
        IZZA.on('pvp-cop-damage',p=>{
          if(DUEL.mid) applyPvpDamageSegments((p&&p.segs)|0 || 1);
        });
      }
      ...
      const startGo=()=>{
        showCountdown(3,()=>{
          DUEL.canFire=true;
          showBanner(`1v1 vs ${DUEL.oppName}`,'round');
          emitRoundStart(1);
        });
      };
      ...
      if(cvs){
        ...
        setTimeout(()=>{ hold.remove(); startGo(); },5000);
      }else{
        setTimeout(startGo,5000);
      }
    });
  }
  ...
  function showCountdown(n=3,onGo){
    ...
    Object.assign(label.style,{
      ...
      top:(rect.top+rect.height/2-34)+'px',
    ...
    setTimeout(function tick(){
      if(cur>0){
        ...
        setTimeout(()=>{ host.remove(); onGo&&onGo(); },650);
      }
    },500);
  }
  ...
  IZZA.on?.('render-post',({dtSec})=>{
    try{
      ...
      const dt = Math.min(1, Math.max(0.08, (dtSec || 0.016) * 12));
      DUEL.oppRender.x += (DUEL.opp.x - DUEL.oppRender.x) * dt;
      DUEL.oppRender.y += (DUEL.opp.y - DUEL.oppRender.y) * dt;
      DUEL.oppRender.facing = DUEL.opp.facing || DUEL.oppRender.facing;

      if(DUEL.oppSprite){
        DUEL.oppSprite.x = DUEL.oppRender.x;
        DUEL.oppSprite.y = DUEL.oppRender.y;
        DUEL.oppSprite.facing = DUEL.oppRender.facing;
      }
      ...
      if(mini){
        ...
        if(ctx){
          ...
          const gx=Math.floor(DUEL.oppRender.x/api.TILE), gy=Math.floor(DUEL.oppRender.y/api.TILE);
      ...
      const api=IZZA.api, cvs=document.getElementById('game');
      if(!api || !cvs) return;
      ...
      if(DUEL.oppCops && DUEL.oppCops.length){
        ...
        for(const c of DUEL.oppCops){
          ...
          if(pack && pack.img){
            ...
            ctx.fillRect(sx+S*.18,sy+S*.18,S*.64,S*.64);
          }
        }
        ctx.restore();
      }
      ...
      if(DUEL.oppSprite){
        const sx=(DUEL.oppRender.x-api.camera.x)*scale, sy=(DUEL.oppRender.y-api.camera.y)*scale;
        const weapon=DUEL.oppEquipped || 'hand';

        if(weapon !== 'hand'){
          const w=Math.floor(S*.30), h=Math.floor(S*.12);
          if(weapon==='bat') ctx.fillStyle='#8b5a2b';
            ...
            ctx.fillStyle='#5b7d61';
            ctx.fillRect(sx+S*.45,sy+S*.34,Math.floor(S*.12),Math.floor(S*.12));
          }

          if(weapon !== 'grenade'){
            const f=DUEL.oppRender.facing || 'down';
            if(f==='down') ctx.fillRect(sx+S*.55,sy+S*.65,w,h);
            else if(f==='up') ctx.fillRect(sx+S*.10,sy+S*.25,w,h);
            else if(f==='left') ctx.fillRect(sx+S*.10,sy+S*.55,w,h);
            else ctx.fillRect(sx+S*.60,sy+S*.55,w,h);
          }
        }
      }
      ...
      if(DUEL.effects.length){
        ...
        for(let i=DUEL.effects.length-1;i>=0;i--){
          const e=DUEL.effects[i];
          e.t += dtSec ? dtSec*1000 : 16;
          const alpha=1-(e.t/e.life);

          if(alpha <= 0){
            DUEL.effects.splice(i,1);
            continue;
          }
          ...
          if(e.kind==='tracer'){
            ...
            ctx.lineWidth=2;
            const sx1=(e.x1-api.camera.x)*scale, sy1=(e.y1-api.camera.y)*scale;
            const sx2=(e.x2-api.camera.x)*scale, sy2=(e.y2-api.camera.y)*scale;
            ctx.beginPath();
            ctx.moveTo(sx1,sy1);
            ctx.lineTo(sx2,sy2);
            ctx.stroke();
            ...
            const sx=(e.x-api.camera.x)*scale, sy=(e.y-api.camera.y)*scale;
