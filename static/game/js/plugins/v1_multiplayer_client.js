/**
 * IZZA Multiplayer Client — v1.6 (Pi auth + friend search + HUD hide)
 * - Uses Pi username from /me (already backed by your Pi token flow).
 * - Searches only users who completed auth (server enforces).
 * - Hides A/B/I HUD buttons while lobby open; shows on close or match start.
 * - Strong typing guard (I/A/B keys do nothing while typing or lobby visible).
 * - Path auto-fallback: supports /izza-game/api/mp and /izza-game/api/mp/mp
 * - WS optional; REST works fine if WS unavailable.
 */
(function () {
  const BUILD = 'v1.6-mp-client';
  ...
  let me = null, friends = [];
  let lobby = null, ui = {}, notifHost = null, mounted = false, notifTimer = null;
  ...
  // Simple HUD show/hide for A/B/I
  const HUD_IDS = ['btnA', 'btnB', 'btnI'];
  ...
  function lobbyVisible() {
    ...
    return !!(h && h.style.display && h.style.display !== 'none');
  }
  ...
  async function loadMe() {
    // Server should return { username, inviteLink, ranks?, ws_url? }
    me = await jget('/me');
    return me;
  }
  async function loadFriends() {
    // Server returns users who completed auth & are your friends
  ...
  async function searchFriendsServer(q) {
    // Server must filter to users who finished auth.html; client just displays
  ...
  async function refreshRanks() {
    try {
      ...
      if (res && res.ranks) { (me || (me = {})).ranks = res.ranks; paintRanks(); }
    } catch {}
  }
  ...
  function paintRanks() {
    ...
    const set = (id, key) => {
      ...
      const s = $('span', el); if (s) s.textContent = `${r.w}W / ${r.l}L`;
    };
  ...
  function paintFriends(list) {
    ...
    host.innerHTML = '';
    if (!list.length) {
      const e = document.createElement('div'); e.className = 'meta'; e.style.opacity = '.8';
      e.textContent = 'No friends yet. Search a Pi username above or share your invite link.'; host.appendChild(e); return;
    }
  ...
  function paintSearchResults(q, results, usedFallback = false) {
    ...
    if (q && q.length >= CFG.minChars) {
      ...
      row.innerHTML = `<div><div>${q}</div><div class="meta">Not found — send a friend request?</div></div><button class="mp-small" data-request="${q}">Add Friend</button>`;
  ...
  function connectWS(urlOverride) {
    ...
    ws.addEventListener('message', (evt) => {
      ...
      if (msg.type === 'presence') updatePresence(msg.user, !!msg.active);
      ...
      else if (msg.type === 'notify.invite') showBanner(
        ...
        setHUDHidden(false); // HUD back in a duel
  ...
  function stopAll(e) { e.preventDefault?.(); e.stopPropagation?.(); e.stopImmediatePropagation?.(); return false; }
  function hotkeyGuard(e) {
    // If the lobby is visible OR you’re typing in it, block I/A/B
    if (!lobbyVisible() && !isTypingTarget(e.target)) return;
  ...
  function bootObserver() {
    ...
    obs = new MutationObserver(() => {
      ...
      // Toggle HUD with visibility
      setHUDHidden(!!visible);
  ...
  // -------- LISTEN FOR MODAL OPEN/CLOSE (from building plugin) --------
  window.IZZA?.on?.('ui-modal-open', (evt) => {
    if (evt?.id === 'mpLobby') setHUDHidden(true);
  });
  window.IZZA?.on?.('ui-modal-close', (evt) => {
    if (evt?.id === 'mpLobby') setHUDHidden(false);
  });
  ...
  async function start() {
    try {
      ...
      // Strong guards across all keyboard phases
