 ...
 * - NEW: automatically append short-lived token (?t=) to all requests
 */
(function(){
  const BUILD='v1.6-mp-client+strong-guard+shield+token';
  ...
  function installShield(){
    ...
    const btnIds=['#btnA','#btnB','#btnI'];
    // disable HUD buttons + swallow
    hudEls=btnIds.map(id=>document.querySelector(id)).filter(Boolean);
  ...
  function mountLobby(host){
    ...
    if(search){
      const run = debounced(async ()=>{
        ...
        try{
          const list = await searchPlayers(q); // players = completed character creation
