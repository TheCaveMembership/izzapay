// Multiplayer Building & Lobby — v1.7 (fix $ refs, passive:false, wider range)
(function(){
  ...
  function anchors(api){
    ...
    const bW=10,bH=6;
    const bX = ((un.x0+un.x1)/2|0) - (bW/2|0);
    const bY = un.y0 + 5;
  ...
  function ensureModal(){
    ...
    // temporary visual queue text; real queue handled by client plugin
  ...
  // Slightly wider proximity so B is easier to hit
  function inRange(){
    const {gx,gy}=playerGrid();
    const s=buildingSpot(api);
    return manhattan(gx,gy, s.gx, s.gy) <= 2; // was 1
  }
  ...
  IZZA.on('ready', (a)=>{
    api=a;
    // IMPORTANT: don't use passive:true here; we want preventDefault to work
