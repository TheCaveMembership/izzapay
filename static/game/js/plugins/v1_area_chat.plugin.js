...
(function(){
  ...
  function ensureUI(){
    ...
    bar.innerHTML = `
      ...
      <input id="chatInput" type="text" placeholder="Type…" autocomplete="off" autocapitalize="sentences" spellcheck="false" inputmode="text"
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if(block.has(k) || k.length===1){
      e.preventDefault();             // prevent core shortcut default
      e.stopImmediatePropagation();   // stop capture/bubble listeners
      e.stopPropagation();
    }
  }, true);
  ...
  // ---- bubbles (DOM overlay anchored to sprites) ----
  ...
  // keep bubbles stuck to heads during movement/camera pan
  IZZA?.on?.('render-post', ()=>{
    try{
  ...
  // also on scroll/resize/orientation (mobile)
  ...
  function sendChat(text){
    ...
    showBubble(uname, text);                                    // local bubble above *your* head
  }
  ...
  Net.on('chat-say', async (m)=>{
    ...
    showBubble(m.from, txt); // viewer-local (translated) text above sender
  });
