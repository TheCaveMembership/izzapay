...
function bindInside(){
  ...
  // btnMint: full handler with value-gated server debit
btnMint && btnMint.addEventListener('click', async ()=>{
  // === [A] Prefight / UI validations =======================================
  ...
  if (!STATE.currentSVG){
    craftStatus.textContent = 'Add/Preview SVG first.';
    return;
  }

  // read selling options (unchanged)
  ...
  // === [B] Server-enforced debit (value-gated credit) =======================
  // calcDynamicPrice() must return { pi, ic } for this item
  const totals = calcDynamicPrice(); // { pi, ic }
  try{
    const r = await fetch(gameApi('/api/crafting/ic/debit'), {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ cost_pi: totals.pi })   // server converts π → required IC
    });
    const j = await r.json();

    // Failure cases: include “insufficient_credit_value” (value too low)
    if (!r.ok || !j || !j.ok){
      if (j && (j.reason === 'insufficient_credit_value')) {
        craftStatus.textContent =
          `This item costs ${totals.ic.toLocaleString()} IC, ` +
          `but your available crafting credits don’t have enough value. ` +
          `Earn or purchase a higher-tier crafting credit to mint this item.`;
      } else if (j && j.reason === 'insufficient_credits') {
        craftStatus.textContent = 'Not enough crafting credits. Please buy or earn more.';
      } else if (j && j.reason === 'auth_required') {
        craftStatus.textContent = 'Please sign in again.';
      } else {
        craftStatus.textContent = 'Unable to mint right now. Please try again.';
      }
      return;
    }

    // Optional: if API returns a balance mirror, you can sync local counters here
    // if (typeof j.balance === 'number') { STATE.mintCredits = j.balance; }
  } catch (e){
    craftStatus.textContent = 'Network error. Please try again.';
    return;
  }

  // === [C] Normalize + inject into game (unchanged) =========================
  try{
    ...
    if (injected && injected.ok){
      // === [D] Success UX + local credit mirrors ============================
      craftStatus.textContent = 'Crafted ✓';

      // burn local credit mirror (legacy counter / package)
      ...
      // persist mirrors / refresh badges immediately
      setCraftingCredits(STATE.mintCredits | 0, 'burn');   // allow downgrade only on burn
      updateTabsHeaderCredits();                           // updates “Create Item” badge
      _syncVisualsTabStyle();                              // toggle visuals enabled state

      // === [E] Persist crafted item on server; capture craftedId ============
      ...
      // === [H] Optional merchant handoff to IZZA Pay ========================
