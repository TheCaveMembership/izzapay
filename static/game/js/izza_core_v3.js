...
function updateCops(dtSec, nowMs){
  let __reinforcedThisTick = false;
  ...
  for(const c of cops){
    ...
    // Tiered reinforcement toward 5★ (guarded so it happens at most once per frame)
    if (!__reinforcedThisTick && nowMs >= c.reinforceAt && player.wanted < 5){
      const curLevel  = player.wanted|0;
      const nextLevel = Math.min(5, curLevel + 1);
      setWanted(nextLevel);
      maintainCops();
      const nextAt = nowMs + _reinforceIntervalFor(nextLevel);
      for (const k of cops) { k.reinforceAt = nextAt; }
      __reinforcedThisTick = true;
    }
  }
}
