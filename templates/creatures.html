...
<head>
  ...
  <style>
    ...
    /* surgical: minting spinner + green consent note */
  ...
  <div class="wrap">
    ...
    <div class="grid">
      <div class="col box">
        ...
        <!-- NEW: collection stats block under mint -->
        <div class="box" id="collStatsBox" style="margin-top:10px">
          <h3 style="margin:0 0 6px">Collection Stats</h3>
          <div id="collStats" class="small-muted">Loading…</div>
        </div>
        <!-- /NEW -->
      </div>
      ...
      <div class="col box">
        ...
        <div class="row">
          ...
          <!-- NEW: burn button in habitat summary -->
          <button class="btn" id="burnLiveBtn">BURN</button>
          <!-- /NEW -->
...
(function(){
  ...
  const collStatsBox = document.getElementById('collStatsBox'); // NEW
  const collStats = document.getElementById('collStats');       // NEW
  const burnLiveBtn = document.getElementById('burnLiveBtn');   // NEW

  // Expanded preview pool…
  ...
  function buildPreviewSkins(){
    ...
    const step = 3;
    const picked = [];
    for(let i=0;i<seeds.length;i+=step) picked.push(seeds[i]);
    return picked;
  }
  ...
  // NEW: collection stats fetcher
  async function refreshStats(){
    try{
      const j = await fetch('/api/creatures/stats').then(r=>r.json());
      const total = j.total_minted || 0;
      const burned = j.total_burned || 0;
      const alive = Math.max(0, total - burned);
      const rc = j.rarity_counts || {};
      collStats.innerHTML =
        `Hatched: <strong>${total}</strong> &nbsp;•&nbsp; Burned: <strong>${burned}</strong> &nbsp;•&nbsp; Alive: <strong>${alive}</strong><br>` +
        `Rarities minted — Common: ${rc.common||0}, Uncommon: ${rc.uncommon||0}, Rare: ${rc.rare||0}, Epic: ${rc.epic||0}, Legendary: ${rc.legendary||0}`;
    }catch(e){
      collStats.textContent = 'Stats unavailable';
    }
  }
  // /NEW

  // Load latest & balances & stats on enter
  (async ()=>{
    try{
  ...
  setInterval(refreshStats, 10000); // NEW
  ...
  mintBtn.addEventListener('click', async ()=>{
    ...
    const previewSkins = (function(){
      const palettes = ["gold","violet","turquoise","rose","lime","sapphire","ember","obsidian","mint"];
      const patterns = ["speckle","stripe","swirl","mosaic","metallic"];
      const rarHints = ["LEG","EP","RARE","UN","COM"];
      const seeds = [];
      for(const r of rarHints){ for(const p of palettes){ for(const t of patterns){ seeds.push(`${r}-${p}-${t}`); }}}
  ...
  // NEW: Burn from live panel
  burnLiveBtn.addEventListener('click', async ()=>{
    if(!lastCode){ alert('Mint or select a creature first'); return; }
    const gate = await ensureUserAndWalletGate(); if(!gate){ return; }
    ...
    if(!ok) return;
    const resp = await fetch('/api/creatures/burn', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ code:lastCode, owner_pub:gate.pub })
    }).then(r=>r.json()).catch(()=>({ok:false}));
    if(resp && resp.ok){
      alert('Burned. Farewell, brave creature.');
      await refreshLive();
      await refreshStats();
      document.getElementById('btnMyCreatures').click();
    }else{
      alert(resp && resp.error ? String(resp.error) : 'Burn failed');
    }
  });
  // /NEW
  ...
  document.getElementById('btnMyCreatures').addEventListener('click', async ()=>{
    ...
    (j.items||[]).forEach(it=>{
      ...
      burnBtn.addEventListener('click', async ()=>{
        const active = await fetchActiveWallet(USER_CANON);
        const pub = active && active.pub || '';
        if(!pub){ alert('Open your IZZA wallet once to set an active wallet, then return.'); return; }
        const ok = confirm('You are about to Burn this NFT! Are you sure you want to send this IZZA Creature back into the hands of IZZA gods?');
        if(!ok) return;
        const resp = await fetch('/api/creatures/burn', {
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ code: it.code, owner_pub: pub })
        }).then(r=>r.json()).catch(()=>({ok:false}));
        if(resp && resp.ok){
          alert('Burned. IZZA gods accept your offering.');
...
<!-- Global friends and notifications overlay, unchanged -->
<script>
